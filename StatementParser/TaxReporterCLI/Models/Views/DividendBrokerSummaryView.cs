...
namespace TaxReporterCLI.Models.Views
{
    public class DividendBrokerSummaryView
    {
        ...
        public DividendBrokerSummaryView(IList<DividendTransactionView> transactions, Broker broker, RegionInfo country)
        {
            this.Broker = broker;
            this.Country = country;
            this.Currency = transactions.FirstOrDefault().Transaction.Currency;

            var brokerTransactions = transactions.Where(i => i.Transaction.Broker == broker && (i.Transaction as DividendTransaction).TaxCountry.Equals(country));
